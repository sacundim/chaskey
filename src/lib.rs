...
/// The Chaskey round function.  $v should be an array or slice of
/// four `u32` elements.
macro_rules! round {
    ($v: expr) => {
...
macro_rules! permute4 {
    ($v: expr) => {
        round!($v); round!($v); round!($v); round!($v);
    }
}

/// The Chaskey permutation.  $v should be an array or slice of four
/// `u32` elements.
macro_rules! permute8 {
    ($v: expr) => {
        permute4!($v); permute4!($v);
    }
}

/// The Chaskey-12 permutation.  $v should be an array or slice of four
/// `u32` elements.
macro_rules! permute12 {
    ($v: expr) => {
        permute8!($v); permute4!($v);
    }
}

/// The Chaskey-LTS permutation.  $v should be an array or slice of
/// four `u32` elements.
macro_rules! permute16 {
    ($v: expr) => {
        permute8!($v); permute8!($v);
    }
}

const C: [u32; 2] = [0x00, 0x87];

/// The Chaskey key schedule function.
macro_rules! times_two {
    ($k: expr) => {
        [$k[0].wrapping_shl(1) ^ C[$k[3].wrapping_shr(31) as usize],
         $k[1].wrapping_shl(1) ^ $k[0].wrapping_shr(31),
         $k[2].wrapping_shl(1) ^ $k[1].wrapping_shr(31),
         $k[3].wrapping_shl(1) ^ $k[2].wrapping_shr(31)]
    }
}

macro_rules! merge_u32x4 {
    ($v: expr, $m: expr) => {
        $v[0] ^= $m[0];
        $v[1] ^= $m[1];
        $v[2] ^= $m[2];
...
macro_rules! merge_u8x16 {
    ($v: expr, $m: expr) => {
        $v[0] ^= LittleEndian::read_u32(&$m[0..4]);
        $v[1] ^= LittleEndian::read_u32(&$m[4..8]);
        $v[2] ^= LittleEndian::read_u32(&$m[8..12]);
        $v[3] ^= LittleEndian::read_u32(&$m[12..16]);
    }
}


pub fn chaskey_subkeys(key: [u32; 4]) -> ([u32; 4], [u32; 4]) {
    let k1: [u32; 4] = times_two!(key);
    let k2: [u32; 4] = times_two!(k1);
    (k1, k2)
}

pub fn chaskey(msg: &[u8], 
               key: [u32; 4], 
               k1: [u32; 4], 
               k2: [u32; 4])
               -> [u32; 4] {
...
impl Chaskey {
    ...
    pub fn write(&mut self, bytes: &[u8]) {
        for byte in bytes.iter() {
            if self.i % 16 == 0 && self.i != 0 {
                merge_u8x16!(self.state, self.buf);
                permute8!(self.state);
            }
    ...
    pub fn finish(&self) -> [u32; 4] {
        ...
        if buflen == 0 && self.i != 0 {
            merge_u8x16!(result, self.buf);
            merge_u32x4!(result, self.k1);
            permute8!(result);
            merge_u32x4!(result, self.k1);
        } else {
            let mut last = [0u8; 16];
            ...
            last[buflen] = 0x01;
            merge_u8x16!(result, last);
            merge_u32x4!(result, self.k2);
            permute8!(result);
            merge_u32x4!(result, self.k2);
        }
        result
...
impl Hasher for Chaskey {
    ...
    fn finish(&self) -> u64 {
        let tag = Chaskey::finish(&self);
        (tag[0] as u64) | (tag[1] as u64).wrapping_shl(32)
    }
}
...
#[cfg(test)]
mod tests {
    use super::{chaskey, chaskey_subkeys, Chaskey};
    ...
    fn test_oneshot() {
        ...
        for i in 0..64 {
            message[i] = i as u8;
            let tag = chaskey(&message[0..i], KEY, k1, k2);
            println!("Iteration = {}", i);
